        ...
        <main className="flex-grow p-4 sm:p-8 max-w-7xl w-full mx-auto pb-24">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'media' && (
              <MediaLibraryPanel 
                mediaItems={media}
                onUpload={(name, url, folder, size, type) => { firestore.addMediaItem(name, url, folder, size, type); reloadData(); triggerToast('File uploaded to Media Library.'); }}
            ...
            {activeTab === 'messages' && (
              <MessagesPanel 
                messages={messages}
                onReply={(id) => { firestore.replyToMessage(id); reloadData(); triggerToast('Reply transmitted regarding inquiry.'); }}
            ...
            <tbody className="divide-y divide-brand-clay/15 text-brand-charcoal/85">
              {staffUsers.map((u) => (
                <tr key={u.uid} className="hover:bg-brand-beige/10 transition-colors">
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2.5">
                      <img src={u.profileImage || "https://i.ibb.co/mV2JMRFb/Pngtree-default-male-avatar-5939655.png"} className="w-8 h-8 rounded-full object-cover border border-brand-clay/15" />
                      <div>
